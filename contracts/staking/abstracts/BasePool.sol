...
abstract contract BasePool is IPool, ReentrancyGuard {
     ...
     * @param _depositId Deposit id to be unstaked
     * @param _newLockUntil new lock timestamp
     */
    function extendLockPeriod(uint256 _depositId, uint256 _newLockUntil)
        external
    {
        _extendLockPeriod(msg.sender, _depositId, _newLockUntil);
    }

    /**
     * @notice Shorten lock period
     * @param _depositId Deposit id to be unstaked
     * @param _newLockUntil new lock timestamp
     */
    function shortenLockPeriod(uint256 _depositId, uint256 _newLockUntil)
        external
    {
        _shortenLockPeriod(msg.sender, _depositId, _newLockUntil);
    }
     ...
     * @param _user User address
     * @param _depositId deposit ID to unstake from, zero-indexed
     * @param _newLockUntil new timestamp to lock until
     */

    function _extendLockPeriod(address _user, uint256 _depositId, uint256 _newLockUntil) internal virtual nonReentrant {
        require(
            _newlockUntil > block.timestamp,
            "Invalid lock interval"
        );
        if (_newlockUntil >= block.timestamp + 365 days)
            _newlockUntil = block.timestamp + 365 days;

        UserInfo storage user = users[_user];
        require(user.deposits[_depositId].exists, "Deposit not found");
        Deposit storage stakeDeposit = user.deposits[_depositId];
    ...
    function _shortenLockPeriod(address _user, uint256 _depositId, uint256 _newLockUntil) internal virtual nonReentrant {
        require(
